...
def prepare_customers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # First provide a list of regions as it is.
    # This allows you to see if changes were implemented successfully
    uncleaned_column_list = region_column.unique().tolist()
    LOG.info(f"Regions before column cleaning are {uncleaned_column_list}")
    ...
    # Strip any sub-region indicated in the region column using str property and split method.
    stripped_2 = stripped.str.split("-").str[0]

    # .str.title() capitalizes the first letter of each word in each value.
    # So "east", "EAST", and " East " all become "East".
    titled = stripped_2.str.title()
    ...
    LOG.info("Customers Prep 2. Convert customer JoinDate the datatype to datetime")
    df["JoinDate"] = pd.to_datetime(df["JoinDate"], errors="coerce")

    LOG.info("Customers Prep 3. Remove duplicate rows")
    ...
    LOG.info(f"  Customers prepared: {after} rows")
    return df
...
def prepare_products(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Summarize information about the UnitPrice Column
    LOG.info(f"UnitPrice original data type:{price_column.dtype}")
    LOG.info(f"UnitPrice converted data type:{price_numeric.dtype}")
    LOG.info(f"  Non-numeric prices replaced with NaN: {bad_prices}")
    ...
    LOG.info("Products Prep 3. Drop product category in the ProductName Column")
    product_name = df["ProductName"].str.split("-").str[1]
...
def prepare_sales(
    ...
    LOG.info("Sales Prep 7. Remove duplicate rows")

    # Drop the CampaignID column since it does not link to any table.
