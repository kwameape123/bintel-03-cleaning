"""The purpose of this script is to ensure that our products table
is clean and ready for down stream analysis. This script will perform the following tasks:
1. Remove any duplicate records from the products table.
2. Address any missing values in the products table.
3. Ensure that the data types of each column are appropriate for analysis.
4. Address any issues related with outliers in the products table.
5. Standardize the format of the data in the products table.
"""
...
def standardize_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "productname" in df.columns:
        name_parts = df["productname"].astype(str).str.split("-", n=1, expand=True)
        df["categoryduplicate"] = name_parts[0]
        df["name"] = name_parts[1].fillna("")
        LOG.info("Split 'productname' column into 'categoryduplicate' and 'name'.")
    df = df.drop(columns=["categoryduplicate", "productname"])
    df.insert(df.columns.get_loc("category") + 1, "name", df.pop("name"))
...
def main() -> None:
    LOG.info("==================================")
    LOG.info("STARTING prep_products_day.py")
    LOG.info("==================================")
